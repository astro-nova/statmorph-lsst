...
from astropy.convolution import convolve, Box2DKernel, convolve_fft, Gaussian2DKernel
...
def _interpolate_missing_pixels(image: np.ndarray, mask: np.ndarray, interpolator='pandas') -> np.ndarray:
    ...
    Returns:
        image_interp: NxN image array with interpolated values over the masked regions
        mask_interp: NxN mask with only the non-interpolated bad pixels
    """
...
class SourceMorphology(object):
    ...
    @lazyproperty
    def _cutout_stamp_masknan(self):
        """Same as _cutout_stamp_maskzeroed, but setting bad pixels to NaN instead of zero.
        This is better for convolution as masked pixels are now properly ignored rather than 
        bleeding into adjacent pixels.
        """
        return np.where(~self._mask_stamp,
                        self._image[self._slice_stamp], np.nan)
    ...
    @lazyproperty
    def _cutout_stamp_interp(self):
        """
        Same as _cutout_stamp_maskzeroed, except we interpolate 
        over the mask where possible (where the mask is enclosed
        by valid values along one of two dimensions). This is 
        only used to construct segmentation maps to ensure that 
    ...
    def _segmap_gini(self):
        ...
        cutout_smooth = ndi.gaussian_filter(
            self._cutout_stamp_interp, petro_sigma, mode='constant', cval=0.0)
    ...
    def _sky_smoothness_residual(self):
        ...
        bkg = self._cutout_stamp_masknan[self._slice_skybox]
        if bkg.size == 0:
            assert self.flag == 2
            return None
        ...
        bkg_smooth = convolve(bkg, Box2DKernel(boxcar_size), mask=np.isnan(bkg))
    ...
    def _smoothness_residual(self):
        ...
        """
        image = self._cutout_stamp_masknan.copy()
        boxcar_size = int(self._petro_fraction_cas * self.rpetro_circ)
        image_smooth = convolve_fft(image, Box2DKernel(boxcar_size), mask=self._mask_stamp, boundary='wrap')
    ...
    def _smoothness_func(self, s_type):
        ...
        image = self._cutout_stamp_maskzeroed
        image_diff = self._smoothness_residual.copy()
        ...
        # Substructure, as defined in Sec. 3.5.2 of Sazonova et al. (in prep)
            ...
            residual_std = sigma_clipped_stats(image_diff, mask=self._mask_stamp)[2]
            clump_size = 5
            clump_segmap = detect_sources(image_diff, 1.5*residual_std, npixels=int(clump_size), connectivity=4)
            self._clump_segmap = clump_segmap
            if clump_segmap is None:
                return 0.0  # no clumps detected
    ...
    def _segmap_mid_main_clump(self, q):
        ...
        # Get the interpolated cutout with background masked, and set negative pixels to zero
        stamp = self._cutout_stamp_interp
        stamp[stamp < 0] = 0.0
        segmap_stamp = self._segmap.data[self._slice_stamp]
        stamp[segmap_stamp == 0] = 0.0
        above_threshold = stamp >= threshold
    ...
    def _cutout_mid_smooth(self):
        ...
        """
        image_smooth = convolve_fft(self._cutout_mid, Gaussian2DKernel(self._sigma_mid),
                                    mask=self._mask_stamp_no_bg, boundary='wrap')
        return image_smooth
    ...
    def _watershed_mid(self):
        ...
        peaks = skimage.feature.peak_local_max(
            self._cutout_mid_smooth, num_peaks=100, threshold_rel=0.05)
    ...
    def _segmap_shape_asym(self):
        ...
        # Use the interpolated image for this (sometimes masked streaks)
        # cause a break in the segmap
        stamp = self._cutout_stamp_interp
        ...
        image_smooth = ndi.uniform_filter(
            stamp, size=self._boxcar_size_shape_asym)
    ...
    def isophote_asymmetry(self):
        ...
        for level in self._asymmetry_isophotes:
            ...
            asym = self._asymmetry_function(self._asymmetry_center, isophote_mask, 'isophote')
